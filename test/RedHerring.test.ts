...
describe('Red Herrings (Fake Clues) Feature', () => {
    ...
    const categoriesNominal4x4: CategoryConfig[] = [
        { id: 'Person', type: CategoryType.NOMINAL, values: ['Alice', 'Bob', 'Charlie', 'Diana'] },
        { id: 'Color', type: CategoryType.NOMINAL, values: ['Red', 'Blue', 'Green', 'Yellow'] },
        { id: 'Drink', type: CategoryType.NOMINAL, values: ['Tea', 'Coffee', 'Soda', 'Water'] },
        { id: 'Pet', type: CategoryType.NOMINAL, values: ['Dog', 'Cat', 'Bird', 'Fish'] }
    ];
    ...
    describe('Safe Max Red Herrings Bounds & Layout Edge Cases', () => {
        ...
        it('should calculate exact safe bounds for multi-category puzzles', () => {
            expect(getSafeMaxRedHerrings(categoriesNominal3x4)).toBe(1);
            expect(getSafeMaxRedHerrings(categoriesNominal4x4)).toBe(2);
    ...
    describe('Configuration Validation & Boundary Enforcement', () => {
        ...
        it('should throw ConfigurationError when requesting more red herrings than safeMax on 3x4 nominal', () => {
            const gen = new Generator(42);
            const safeMax = getSafeMaxRedHerrings(categoriesNominal3x4); // 1
            expect(() => {
    ...
    describe('Exact Maximum Safe Bounds Generation', () => {
        ...
        it('should successfully generate at EXACT safeMax on 3-category nominal puzzle (K=1)', () => {
        ...
        it('should successfully generate at EXACT safeMax on 4-category nominal puzzle (K=2)', () => {
            const gen = new Generator(203);
            const safeMax = getSafeMaxRedHerrings(categoriesNominal4x4); // 2
            const puzzle = gen.generatePuzzle(categoriesNominal4x4, undefined, { redHerrings: safeMax });
    ...
    describe('Multiple Red Herrings (K >= 2) Combinatorial Unicity', () => {
        it('should generate 2 mutually compatible red herrings on a 3x4 puzzle and verify combinatorial unicity', () => {
            ...
            // Test all size-N combinations of all presented clues
            const allClues = puzzle.clues;
            const N = puzzle.validClues.length;
            const combs = getCombinations(allClues, N);

            let validSolvedSubsets = 0;
                ...
                if (!contradiction) {
                    ...
                    for (const c1 of categories3x4) {
                        for (const v1 of c1.values) {
                            ...
                            if (!fullySolved) break;
                        }
                        if (!fullySolved) break;
                    }
                    if (fullySolved) validSolvedSubsets++;
                }
            }

            // Exactly 1 size-N subset fully solves the board with 0 contradictions (the master validClues)
            expect(validSolvedSubsets).toBe(1);
        });

        it('should generate 3 mutually compatible red herrings on a 3x4 ordinal puzzle', () => {
            const gen = new Generator(400);
            const puzzle = gen.generatePuzzle(categories3x4, undefined, { redHerrings: 3 });
