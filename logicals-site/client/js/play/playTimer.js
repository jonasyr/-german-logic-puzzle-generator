...
/**
 * Duel clock anchored to the room's shared start instant. Covering the local
 * board never changes the score; only solving freezes it.
 */
export function createAuthoritativeTimer(startsAt, onTick) {
    let stoppedAt = null;
    let handle = null;

    const elapsedMs = () => Math.max(0, (stoppedAt ?? Date.now()) - startsAt);
    const repaint = () => onTick(elapsedMs());
    ...
    return {
        elapsedMs,
        start() { repaint(); schedule(); },
        cover() { repaint(); },
        pause() { repaint(); },
        resume() { repaint(); schedule(); },
        stop() {
        ...
        reset(toMs = 0) {
            stoppedAt = startsAt + Math.max(0, toMs);
            stopInterval();
            repaint();
        },
        isRunning: () => stoppedAt === null,
        refresh() { repaint(); if (stoppedAt === null) schedule(); },
    };
}
