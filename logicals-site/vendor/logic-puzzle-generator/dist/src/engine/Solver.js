...
class Solver {
     ...
     */
    getOrdinalRankMap(values) {
        const rankToValue = [...values].sort((a, b) => a - b);
        const valueToRank = new Map(rankToValue.map((v, rank) => [v, rank]));
    ...
    applyClue(grid, clue) {
        ...
        switch (clue.type) {
            ...
            case types_1.ClueType.BETWEEN:
                deductions += this.applyBetweenClue(grid, clue, reasons);
                break;
            case types_1.ClueType.ADJACENCY:
                deductions += this.applyAdjacencyClue(grid, clue, reasons);
                break;
            case types_1.ClueType.OR:
                deductions += this.applyDisjunctionClue(grid, clue, reasons);
                break;
            case types_1.ClueType.ARITHMETIC:
                deductions += this.applyArithmeticClue(grid, clue, reasons);
                break;
        }
        let newDeductions;
        do {
    ...
    applyCrossOrdinalClue(grid, clue, reasons) {
        ...
        const getPossibleRanks = (itemCat, itemVal, ordCat, ordConfig) => {
            const { valueToRank } = this.getOrdinalRankMap(ordConfig.values);
            return ordConfig.values
                .map((v) => ({ val: v, idx: valueToRank.get(v) }))
    ...
    applyAdjacencyClue(grid, clue, reasons) {
        let deductions = 0;
        const categories = grid.categories;
        const ordCategory = categories.find(c => c.id === clue.ordinalCat);
        if (!ordCategory || ordCategory.type !== types_1.CategoryType.ORDINAL)
            return 0;
        ...
        const getNeighbors = (val) => {
            const rank = valueToRank.get(val);
            if (rank === undefined)
                return [];
            const neighbors = [];
            if (rank > 0)
                neighbors.push(rankToValue[rank - 1]);
            if (rank < rankToValue.length - 1)
                neighbors.push(rankToValue[rank + 1]);
            return neighbors;
        };
        const eliminations = (itemCat, itemVal, neighborCat, neighborVal, targetNote) => {
    ...
    applyBetweenClue(grid, clue, reasons) {
        let deductions = 0;
    ...
    applyOrdinalClue(grid, constraint, reasons) {
        ...
        const possibleVals1 = ordCatConfig.values
            .map((v) => ({ val: v, idx: ordRankMap.get(v) }))
    ...
    applyDisjunctionClue(grid, clue, reasons) {
    ...
    applyArithmeticClue(grid, clue, reasons) {
        let deductions = 0;
        const categories = grid.categories;
        const ordCategory = categories.find(c => c.id === clue.ordinalCat);
        if (!ordCategory || ordCategory.type !== types_1.CategoryType.ORDINAL)
            return 0;
        const ordValues = ordCategory.values;
        ...
        const getPossibilities = (cat, val) => ordValues.filter(v => grid.isPossible(cat, val, clue.ordinalCat, v));
        ...
        const valid1 = new Set();
        const valid2 = new Set();
        const valid3 = new Set();
        const valid4 = new Set();
        // Optimized iteration
        ...
        const prune = (cat, val, candidates, initial) => {
            let localDeductions = 0;
