...
exports.getSafeMaxRedHerrings = getSafeMaxRedHerrings;
 ...
 */
function getSafeMaxRedHerrings(categories) {
    const numCats = categories.length;
    const numItems = categories[0]?.values.length ?? 0;
    const hasOrdinal = categories.some(c => c.type === types_1.CategoryType.ORDINAL);
    // 1. Minimum layout requirements
    if (numCats < 2 || numItems < 3)
        return 0;
    if (!hasOrdinal && (numCats < 3 || numItems < 4))
        return 0;
    ...
    if (hasOrdinal) {
        ...
        // Pure nominal categories lack fixed anchor points (unlike ordinals)
        return numCats >= 4 ? Math.min(2, numItems - 2) : 1;
    }
}
...
class Generator {
    ...
    generatePuzzle(categories, target, config = {}) {
        ...
        if (redHerringCount < 0) {
            throw new errors_1.ConfigurationError('Red herring count cannot be negative.');
        }
        if (redHerringCount > 0) {
            const safeMax = getSafeMaxRedHerrings(categories);
            if (safeMax === 0) {
                throw new errors_1.ConfigurationError(`Invalid Configuration: Red Herrings require either at least 1 Ordinal category (>= 3 items) ` +
                    `or at least 3 categories with >= 4 items each to guarantee unambiguous, non-trivial puzzles.`);
            }
            if (redHerringCount > safeMax) {
                throw new errors_1.ConfigurationError(`Invalid Configuration: Requested ${redHerringCount} red herrings, ` +
        ...
        if (redHerringCount > 0) {
            ...
            puzzle.clues = (0, determinism_1.seededShuffleInPlace)([...puzzle.validClues, ...redHerrings], this.random);
        }
        return puzzle;
    }
    /**
        ...
        for (const cat of categories) {
            if (cat.type === types_1.CategoryType.ORDINAL) {
                ...
                if (uniqueValues.size !== cat.values.length) {
                    throw new errors_1.ConfigurationError(`Category '${cat.id}' is ORDINAL but contains duplicate values. Ordinal values must be unique.`);
                }
            }
        }
    ...
    generateWithBacktracking(categories, target, targetCount, maxCandidates, strategy, timeoutMs = 10000, constraints, onTrace) {
        ...
        const search = (currentGrid, currentChain, currentAvailable, overrideStrategy) => {
            ...
            if (overrideStrategy) {
                ...
                orderedCandidates = (0, determinism_1.stableSortInPlace)(candidates, (a, b) => {
                ...
                (0, determinism_1.stableSortInPlace)(candidates, (a, b) => b.score - a.score);
            ...
            if (!overrideStrategy) {
                ...
                if (targetCount) {
                    ...
                    if (progress > expectedProgress) {
                        ...
                        const sortedByWeakness = (0, determinism_1.stableSortInPlace)([...candidates], (a, b) => {
                        ...
                        const sortedByStrength = (0, determinism_1.stableSortInPlace)([...candidates], (a, b) => {
                            if (a.deductions !== b.deductions)
    ...
    createSolution(categories, valueMap, solution, reverseSolution) {
        ...
        for (let i = 1; i < categories.length; i++) {
            ...
            const shuffledValues = (0, determinism_1.seededShuffleInPlace)([...currentCategory.values], this.random);
            let i_shuffled = 0;
    ...
    generateAllPossibleClues(categories, constraints, reverseSolution, valueMap) {
        ...
        for (const ordCategory of categories.filter(c => c.type === types_1.CategoryType.ORDINAL)) {
            const sortedValues = (0, determinism_1.stableSortInPlace)([...ordCategory.values], (a, b) => a - b);
        ...
        if (isAllowed(types_1.ClueType.BETWEEN)) {
            const ordCategories = categories.filter(c => c.type === types_1.CategoryType.ORDINAL);
            for (const ordCat of ordCategories) {
                // Iterate Target Category (Middle)
                for (const targetCat of categories) {
                    if (targetCat.id === ordCat.id)
                        continue;
                    for (const targetVal of targetCat.values) {
                        const baseVal = reverseSolution.get(targetCat.id)?.get(targetVal);
                        if (!baseVal)
                        ...
                        if (!mappings)
                            continue;
                        const targetOrdVal = mappings[ordCat.id];
                        ...
                        for (const otherCat of categories) {
                            if (otherCat.id === ordCat.id)
                                continue;
                            const potentialLowers = [];
                            const potentialUppers = [];
                            for (const otherVal of otherCat.values) {
                                if (otherCat.id === targetCat.id && otherVal === targetVal)
                                    continue;
                                ...
                                if (!otherMap)
                                    continue;
                                const otherOrd = otherMap[ordCat.id];
                                // Strict inequality
                                if (otherOrd < targetOrdVal)
                                    potentialLowers.push(otherVal);
                                if (otherOrd > targetOrdVal)
                                    potentialUppers.push(otherVal);
                            }
                            // Combine
                            for (const lowerVal of potentialLowers) {
                                for (const upperVal of potentialUppers) {
                                    clues.push({
                                        type: types_1.ClueType.BETWEEN,
                                        targetCat: targetCat.id,
                                        targetVal: targetVal,
                            ...
                            for (let j = startJ; j < cat2.values.length; j++) {
                                ...
                                if (rank1 !== undefined && rank2 !== undefined) {
                                    if (Math.abs(rank1 - rank2) === 1) {
                                        clues.push({
                                            type: types_1.ClueType.ADJACENCY,
        ...
        if (isAllowed(types_1.ClueType.OR)) {
            ...
            for (let i = 0; i < targetOrCount; i++) {
                ...
                while (!falseClue && attempts < 10) {
                    ...
                    if (useBinary) {
                        ...
                        falseClue = {
                            ...
                            operator: areMatch ? types_1.BinaryOperator.IS_NOT : types_1.BinaryOperator.IS
                        };
                    }
                    else {
                ...
                if (falseClue) {
                    // Randomize order
                    if (this.random() > 0.5) {
                        clues.push({ type: types_1.ClueType.OR, clue1: trueClue, clue2: falseClue });
                    }
                    else {
                        clues.push({ type: types_1.ClueType.OR, clue1: falseClue, clue2: trueClue });
                    }
                }
            }
        }
        // Generate Arithmetic Clues (Same Difference)
        if (isAllowed(types_1.ClueType.ARITHMETIC)) {
            for (const ordCat of categories) {
                if (ordCat.type !== types_1.CategoryType.ORDINAL)
                    continue;
                ...
                for (const itemCat of categories) {
                    if (itemCat.id === ordCat.id)
                        continue; // Don't compare Ordinal items directly (trivial?)
                    ...
                    for (let i = 0; i < itemValues.length; i++) {
                        for (let j = i + 1; j < itemValues.length; j++) {
                            ...
                            if (diff === 0)
                                continue; // Diff 0 means same value. "Diff between A and B is 0" -> A=B. Trivial if A!=B.
                            if (!diffMap.has(diff))
                                diffMap.set(diff, []);
                    ...
                    for (const [diff, pairs] of diffMap.entries()) {
                        ...
                        for (let i = 0; i < pairs.length; i++) {
                            for (let j = i + 1; j < pairs.length; j++) {
                                ...
                                clues.push({
                                    type: types_1.ClueType.ARITHMETIC,
    ...
    calculateClueScore(grid, target, deductions, clue, previouslySelectedClues, solution, reverseSolution) {
        ...
        switch (clueType) {
            ...
            case types_1.ClueType.UNARY:
                complexityBonus = 1.2;
                break;
            case types_1.ClueType.BETWEEN:
                complexityBonus = 4.0;
                break;
            case types_1.ClueType.ADJACENCY:
                complexityBonus = 3.5;
                break;
            case types_1.ClueType.OR:
                ...
                complexityBonus = 6.0;
                break;
            case types_1.ClueType.ARITHMETIC:
                complexityBonus = 4.5;
                break;
        }
        ...
        if (previouslySelectedClues.length > 0) {
            ...
            const getComplexity = (c) => {
                switch (c.type) {
                    ...
                    case types_1.ClueType.BETWEEN: return 4;
                    case types_1.ClueType.ADJACENCY: return 3;
                    case types_1.ClueType.OR: return 5; // OR clues are generally complex
                    case types_1.ClueType.ARITHMETIC: return 4;
                    default: return 2;
                }
            };
     ...
     */
    checkClueConsistency(clue, solution, reverseSolution, valueMap, categories) {
        ...
        switch (clue.type) {
            ...
            case types_1.ClueType.BETWEEN: {
                ...
                if (midVal === undefined || lowVal === undefined || highVal === undefined)
                    return false;
            ...
            case types_1.ClueType.ADJACENCY: {
                ...
                if (!ordCatConfig)
                    return false;
                const sortedVals = (0, determinism_1.stableSortInPlace)([...ordCatConfig.values], (a, b) => a - b);
     ...
     */
    generateCandidateFalseClues(categories, constraints, reverseSolution = this.reverseSolution, valueMap = this.valueMap, solution = this.solution) {
        const falseClues = [];
        const isAllowed = (type) => !constraints?.allowedClueTypes || constraints.allowedClueTypes.includes(type);
        // 1. Binary Clues
        if (isAllowed(types_1.ClueType.BINARY)) {
                ...
                for (const val1 of cat1.values) {
                    for (const cat2 of categories) {
                        if (cat1.id >= cat2.id)
                            continue;
                        for (const val2 of cat2.values) {
                            for (const op of [types_1.BinaryOperator.IS, types_1.BinaryOperator.IS_NOT]) {
                                const clue = {
                                    type: types_1.ClueType.BINARY,
                                    operator: op,
                                    cat1: cat1.id,
        ...
        for (const ordCat of ordCategories) {
            ...
            if (isAllowed(types_1.ClueType.SUPERLATIVE)) {
                for (const targetCat of categories) {
                    if (targetCat.id === ordCat.id)
                        continue;
                    for (const targetVal of targetCat.values) {
                        for (const op of [types_1.SuperlativeOperator.MIN, types_1.SuperlativeOperator.MAX, types_1.SuperlativeOperator.NOT_MIN, types_1.SuperlativeOperator.NOT_MAX]) {
                            const clue = {
                                type: types_1.ClueType.SUPERLATIVE,
                                operator: op,
                                targetCat: targetCat.id,
            ...
            if (isAllowed(types_1.ClueType.ORDINAL)) {
                for (const item1Cat of categories) {
                    if (item1Cat.id === ordCat.id)
                        continue;
                    for (const item2Cat of categories) {
                        if (item2Cat.id === ordCat.id)
                            continue;
                        for (const item1Val of item1Cat.values) {
                            for (const item2Val of item2Cat.values) {
                                if (item1Cat.id === item2Cat.id && item1Val === item2Val)
                                    continue;
                                for (const op of [types_1.OrdinalOperator.GREATER_THAN, types_1.OrdinalOperator.LESS_THAN, types_1.OrdinalOperator.NOT_GREATER_THAN, types_1.OrdinalOperator.NOT_LESS_THAN]) {
                                    const clue = {
                                        type: types_1.ClueType.ORDINAL,
                                        operator: op,
                                        item1Cat: item1Cat.id,
                ...
                if (ordCat.values.every(v => typeof v === 'number')) {
                    for (const targetCat of categories) {
                        if (targetCat.id === ordCat.id)
                            continue;
                        for (const targetVal of targetCat.values) {
                            for (const filter of [types_1.UnaryFilter.IS_ODD, types_1.UnaryFilter.IS_EVEN]) {
                                const clue = {
                                    type: types_1.ClueType.UNARY,
                                    filter: filter,
                                    targetCat: targetCat.id,
     ...
     */
    generateRedHerrings(categories, validClues, options, reverseSolution = this.reverseSolution, valueMap = this.valueMap, solution = this.solution) {
        ...
        const shuffledCandidates = (0, determinism_1.seededShuffleInPlace)([...stealthyCandidates], this.random);
        // 2. Filter for Unambiguity & Mutual Compatibility
        const selectedRedHerrings = [];
        for (const candidate of shuffledCandidates) {
            ...
            for (const prev of selectedRedHerrings) {
                const grid = new LogicGrid_1.LogicGrid(categories);
            ...
            const N = validClues.length;
            const combs = this.getCombinations(allClues, N);
            let hasAlternativeSolve = false;
            for (const comb of combs) {
                // Skip the master true set
                const isMasterSet = comb.every(c => validClues.includes(c));
                if (isMasterSet)
                    continue;
                const grid = new LogicGrid_1.LogicGrid(categories);
        ...
        if (selectedRedHerrings.length < count) {
            throw new errors_1.ConfigurationError(`Could not generate ${count} mutually compatible red herrings for this puzzle. ` +
